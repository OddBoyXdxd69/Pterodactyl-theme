...
class ServerViewController extends Controller
{
    ...
    public function manage(Request $request, Server $server): View
    {
        ...
        // Check if the panel has at least 1 node configured (allows same-node re-allocation transfer).
        ...
        // Fetch nodes data for server creation
        $nodeData = $this->nodeRepository->getNodesForServerCreation()->toArray();

        // For the server's current node, append its current allocations so they show up in the transfer options
        $server->load('allocations');
        $serverAllocations = $server->allocations->map(function ($allocation) {
            return [
                'id' => $allocation->id,
        ...
        foreach ($nodeData as &$node) {
            if ($node['id'] === $server->node_id) {
                ...
                foreach ($merged as $alloc) {
                    $unique[$alloc['id']] = $alloc;
                }
                $node['allocations'] = array_values($unique);
            }
        }
