            ...
            <form action="" method="POST">
                <div class="box">
                    <div class="box-header with-border">
                        <h3 class="box-title">Captcha Security Settings</h3>
                    </div>
                    <div class="box-body">
                        <div class="row">
                            <div class="form-group col-md-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-muted small">Toggle global captcha security on login, registration, and recovery pages.</p>
                                </div>
                            </div>
                            <div class="form-group col-md-3">
                                ...
                                <div>
                                    @php
                                        $provider = old('recaptcha:provider', config('recaptcha.provider', 'recaptcha'));
                                    @endphp
                                    <select class="form-control" name="recaptcha:provider">
                                        <option value="recaptcha" @if($provider === 'recaptcha') selected @endif>Google reCAPTCHA v2 (Invisible)</option>
                                        <option value="turnstile" @if($provider === 'turnstile') selected @endif>Cloudflare Turnstile (Recommended)</option>
                                        <option value="hcaptcha" @if($provider === 'hcaptcha') selected @endif>hCaptcha (Invisible)</option>
                                    </select>
                                    <p class="text-muted small">Select your preferred captcha security verification provider.</p>
                                </div>
                            </div>
                            <div class="form-group col-md-3">
                                <label class="control-label">Site Key (Website Key)</label>
                                <div>
                                    <input type="text" required class="form-control" name="recaptcha:website_key" value="{{ old('recaptcha:website_key', config('recaptcha.website_key')) }}">
                                    <p class="text-muted small">The public key used by the client-side widget in your templates.</p>
                                </div>
                            </div>
                            <div class="form-group col-md-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-muted small">The private key used for secure server-side token verification.</p>
                                </div>
                            </div>
                        </div>
                        @if($showRecaptchaWarning && $provider === 'recaptcha')
                            <div class="row">
                                <div class="col-xs-12">
                                    <div class="alert alert-warning no-margin">
                                        You are currently using default reCAPTCHA keys. For improved security, it is highly recommended to generate custom keys matching your domain.
                                    </div>
                                </div>
                            </div>
                        @endif
